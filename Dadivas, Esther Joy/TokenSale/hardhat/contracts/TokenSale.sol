...
contract TokenSale is ERC20 {
    ...
    ) ERC20("One Community Token", "OCTK") {
    ...
    function startPresale() public onlyOwner {
        presaleStarted = true;
        presaleEndTime = block.timestamp + 24 hours; //1 days | hours | seconds | minutes | weeks | years
    }
    ...
    function presaleMint() public payable {
        ...
        ); //checks if it is still presale and not ended
        require(
            whitelist.isWhitelisted(msg.sender),
            "You are not whitelisted."
        ); // checks if you are whitelisted
        require(
            presaleMinted[msg.sender] == false,
            "You have already minted your presale tokens."
        ); // checks if you availed already

        uint256 mintPrice = tokensPerAddress * tokenPrice;
        require(msg.value >= mintPrice, "Amount sent is not correct.");

        uint256 tokensWithDecimals = tokensPerAddress * 10 ** 18; //converts to wei
        _mint(msg.sender, tokensWithDecimals);
        balances[msg.sender] += tokensWithDecimals; //updates balance
        _totalSupply += tokensWithDecimals; // updates contract supply
        presaleMinted[msg.sender] = true; //tells u have availed
    }
    ...
    function publicMint(uint256 amountToMint) public payable {
        ...
        require(
            maxTotalSupply >= (_totalSupply + (amountToMint * 10 ** 18)),
            "Cannot exceed maximum total supply."
        );
